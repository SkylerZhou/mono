...
def _validate_fn(model):
    ...
    precision = precision_score(data_list, out_list, average='weighted')
    recall = recall_score(data_list, out_list, average='weighted')
    accuracy = accuracy_score(data_list, out_list)
    print(log_str, "| f1 score: {:.3f}".format(f1), "| precision: {:.3f}".format(precision), "| recall: {:.3f}".format(recall), "| accuracy: {:.3f}".format(accuracy))
